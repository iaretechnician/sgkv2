...
FSteamNetSocket::FSteamNetSocket(ESocketType InSocketType, const FString& InSocketDescription, const FName& InSocketProtocol) :
...
{
	SocketSubsystem = static_cast<FNetSocketsSubsystem*>(ISocketSubsystem::Get(STEAM_NETWORKING_SOCKETS_SUBSYSTEM));
	ISteamNetworkingSockets* SocketInterface = FNetSocketsSubsystem::GetSteamSocketsInterface();
	PollGroup = k_HSteamNetPollGroup_Invalid;
}

FSteamNetSocket::~FSteamNetSocket()
{
...
bool FSteamNetSocket::Close()
{
	...
	STEAM_SDK_IGNORE_REDUNDANCY_START
		...
		{
			UE_LOG(LogSockets, VeryVerbose, TEXT("SteamNetSockets: Socket is already cleaned up, ready for destruction."));
			return true;
		}
	...
	{
		UE_LOG(LogSockets, VeryVerbose, TEXT("SteamNetSockets: Socket interface not found, cannot close."));
		return false;
	}
	...
	if (bIsListenSocket)
	{
		...
		if (bWasSuccessful)
		{
			UE_LOG(LogSockets, Verbose, TEXT("SteamNetSockets: Closed listen socket %u via the API"), InternalHandle);
	...
	else
	{
		...
		if (bWasSuccessful)
		{
			UE_LOG(LogSockets, Verbose, TEXT("SteamNetSockets: Closed socket %u via the API, reason %d"), InternalHandle, ClosureReason);
...
bool FSteamNetSocket::Bind(const FInternetAddr& Addr)
{
	if (Addr.GetProtocolType() == FNetworkProtocolTypes::SteamNetSocketsIP ||
		Addr.GetProtocolType() == FNetworkProtocolTypes::SteamNetSocketsP2P)
	{
		// Because of the SteamAPI, we have to just hold onto the bind address
		// We can't actually do anything with it until later.
		BindAddress = *((FInternetAddrNetSockets*)&Addr);
		return true;
	}
...
bool FSteamNetSocket::Connect(const FInternetAddr& Addr)
{
	ISteamNetworkingSockets* SocketInterface = FNetSocketsSubsystem::GetSteamSocketsInterface();
	...
	if (SocketInterface == nullptr)
	{
		UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: Socket Interface is null, cannot connect"));
		return false;
	}

	if (!Addr.IsValid())
	{
		UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: Connection address is not valid. Cannot connect!"));
		SocketSubsystem->LastSocketError = SE_EINVAL;
		return false;
	}

	FInternetAddrNetSockets SteamAddr = *((FInternetAddrNetSockets*)&Addr);
	if (GetProtocol() == FNetworkProtocolTypes::SteamNetSocketsIP)
	{
	...
	else if (GetProtocol() == FNetworkProtocolTypes::SteamNetSocketsP2P)
	{
	...
	{
		UE_LOG(LogSockets, Verbose, TEXT("SteamNetSockets: Connection to %s initiated"), *Addr.ToString(false));
...
bool FSteamNetSocket::Listen(int32 MaxBacklog)
{
	ISteamNetworkingSockets* SocketInterface = FNetSocketsSubsystem::GetSteamSocketsInterface();
	...
	if (SocketInterface == nullptr)
	{
		UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: Socket interface is null, cannot establish interface for listening!"));
		return false;
	}
...
class FSocket* FSteamNetSocket::Accept(const FString& InSocketDescription)
{
	FSteamNetSocket* NewSocket = static_cast<FSteamNetSocket*>(SocketSubsystem->CreateSocket(FName("SteamClientSocket"), InSocketDescription, GetProtocol()));
	...
	if (BindAddress.GetProtocolType() == FNetworkProtocolTypes::SteamNetSocketsP2P)
	{
...
bool FSteamNetSocket::SendTo(const uint8* Data, int32 Count, int32& BytesSent, const FInternetAddr& Destination)
{
	FInternetAddrNetSockets CurrentAddress;
	...
	{
		FNetSocketsSubsystem::FNetSocketInformation* SocketInfo = SocketSubsystem->GetSocketInfo(Destination);
...
bool FSteamNetSocket::Send(const uint8* Data, int32 Count, int32& BytesSent)
{
	BytesSent = 0;
	...
	if (InternalHandle != k_HSteamNetConnection_Invalid && GetConnectionState() == SCS_Connected)
	{
		...
		switch (FNetSocketsSubsystem::GetSteamSocketsInterface()->SendMessageToConnection(InternalHandle, (void*)Data, Count, SendMode, nullptr))
		{
		case k_EResultOK:
...
bool FSteamNetSocket::Recv(uint8* Data, int32 BufferSize, int32& BytesRead, ESocketReceiveFlags::Type Flags)
{
	BytesRead = -1;
...
bool FSteamNetSocket::RecvRaw(SteamNetworkingMessage_t*& Data, int32 MaxMessages, int32& MessagesRead, ESocketReceiveFlags::Type Flags)
{
	ISteamNetworkingSockets* SocketInterface = FNetSocketsSubsystem::GetSteamSocketsInterface();
	if (SocketInterface == nullptr)
	{
	...
	if (bIsPeeking)
	{
		...
		if (MaxMessages > 1)
		{
			UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: Recv cannot peek more than 1 message on the queue. Was asked for %d"), MaxMessages);
			MaxMessages = 1;
		}
	...
	{
		UE_LOG(LogSockets, Error, TEXT("SteamNetSockets: Recv Connection handle is marked as invalid! Is Listen Socket? %d"), bIsListenSocket);
		MessagesRead = -1;
	}
...
bool FSteamNetSocket::HasPendingData(uint32& PendingDataSize)
{
	if (bHasPendingData)
	{
		...
		{
			UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: HasPendingData flag is raised but the pending data object is null!"));
			bHasPendingData = false;
		}
	}
...
ESocketConnectionState FSteamNetSocket::GetConnectionState()
{
...
void FSteamNetSocket::SetSendMode(int32 NewSendMode)
{
	SendMode = NewSendMode;
}

bool FSteamNetSocket::GetPeerAddress(FInternetAddr& OutAddr)
{
	ISteamNetworkingSockets* SocketInterface = FNetSocketsSubsystem::GetSteamSocketsInterface();
	FInternetAddrNetSockets& SteamAddr = (FInternetAddrNetSockets&)OutAddr;
...
bool FSteamNetSocket::SetNoDelay(bool bIsNoDelay)
{
...
bool FSteamNetSocket::SetLinger(bool bShouldLinger, int32 Timeout)
{
	bShouldLingerOnClose = bShouldLinger;
	return true;
}

void FSteamNetSocket::SetLanOptions()
{
...
bool FSteamNetSocket::SetSendBufferSize(int32 Size, int32& NewSize)
{
	bool bSuccess = false;
	...
	{
		UE_LOG(LogSockets, Warning, TEXT("SteamNetSockets: Setting buffer size requires access to network utilities!"));
		NewSize = -1;
	}
...
bool FSteamNetSocket::SetReceiveBufferSize(int32 Size, int32& NewSize)
{
	// This is 1:1 with SendBufferSize on SteamSockets.
	return SetSendBufferSize(Size, NewSize);
}

bool FSteamNetSocket::Shutdown(ESocketShutdownMode Mode)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::Wait(ESocketWaitConditions::Type Condition, FTimespan WaitTime)
{
	// Unsupported. Can't tell if ready to write or not.
	return false;
}

bool FSteamNetSocket::WaitForPendingConnection(bool& bHasPendingConnection, const FTimespan& WaitTime)
{
	// Unfortunately, this API has no ability to check if there's a pending connection ahead of time.
	return false;
}

bool FSteamNetSocket::SetReuseAddr(bool bAllowReuse)
{
	// Unsupported.
	return true;
}

bool FSteamNetSocket::SetRecvErr(bool bUseErrorQueue)
{
	// Unsupported.
	return true;
}

bool FSteamNetSocket::SetNonBlocking(bool bIsNonBlocking)
{
	// Unsupported. API does not block.
	return true;
}

bool FSteamNetSocket::SetBroadcast(bool bAllowBroadcast)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::JoinMulticastGroup(const FInternetAddr& GroupAddress, const FInternetAddr& InterfaceAddress)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::JoinMulticastGroup(const FInternetAddr& GroupAddress)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::LeaveMulticastGroup(const FInternetAddr& GroupAddress)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::LeaveMulticastGroup(const FInternetAddr& GroupAddress, const FInternetAddr& InterfaceAddress)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::SetMulticastLoopback(bool bLoopback)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::SetMulticastTtl(uint8 TimeToLive)
{
	// Unsupported.
	return false;
}

bool FSteamNetSocket::SetMulticastInterface(const FInternetAddr& InterfaceAddress)
{
	// Unsupported.
	return false;
}
